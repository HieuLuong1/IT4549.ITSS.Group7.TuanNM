...
import { mockUsers as initialUsers, User } from '../../data/mockData';
...
const UserManagement: React.FC = () => {
  ...
  const handleAddUser = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const defaultAvatar = `https://api.dicebear.com/7.x/avataaars/svg?seed=${formData.get('name')}`;
    
    const newUser: User = {
      id: `#FW0${users.length + 1}`,
      name: formData.get('name') as string,
      email: formData.get('email') as string,
      phone: formData.get('phone') as string,
      role: formData.get('role') as any,
      gender: formData.get('gender') as any,
      avatar: avatarFile && avatarFile.size > 0 ? URL.createObjectURL(avatarFile) : defaultAvatar,
      status: 'Active',
      email_verified: false,
      family_id: null,
      role_id: formData.get('role') === 'ADMIN' ? 1 : 2
    };
                  ...
                  <tbody>
                    {currentUsers.map(user => (
                      <tr key={user.id}>
                        ...
                        <td style={{ fontSize: '0.875rem', color: '#64748b' }}>{user.phone}</td>
                        <td style={{ fontSize: '0.875rem', color: '#64748b' }}>{user.email}</td>
                        <td>
                          <div style={{ display: 'flex', justifyContent: 'center' }}>
                            <div className="um-role-badge">
                              {user.role === 'ADMIN' ? 'Người nội trợ' : 'Thành viên'}
                            </div>
                          </div>
                        </td>
                        <td>
            ...
            <SharedModal title={isEditing ? "Chỉnh sửa người dùng" : "Chi tiết người dùng"} onClose={() => { setViewUser(null); setIsEditing(false); }}>
              <div style={{ display: 'flex', gap: '2rem' }}>
                <div style={{ width: '120px', height: '120px', borderRadius: '24px', overflow: 'hidden', backgroundColor: '#F1FAF6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <img src={viewUser.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
                <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <DetailItem label="Họ tên" value={viewUser.name} />
                      <DetailItem label="Vai trò" value={viewUser.role === 'ADMIN' ? 'Người nội trợ' : 'Thành viên'} isBadge />
